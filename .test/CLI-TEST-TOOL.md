# BLACKBOX CLI Test Tool & Batch Test Runner

CLI 工具 + 批量测试运行器，用于从命令行驱动 BLACKBOX GUI 进行端到端测试。仅在 debug 构建下可用。
...
┌─────────────────────────────────────────────────────────────┐
│  AI / 人类                                                   │
│  ├── 直接调用: node scripts/blackbox-cli.mjs <command>     │
│  └── 批量测试: node scripts/run-tests.mjs <test.json>       │
│       └── 内部逐条调用 blackbox-cli.mjs                    │
└────────────────────────┬────────────────────────────────────┘
                         │ NDJSON over Unix socket
                         │ /tmp/blackbox-test.sock
                         v
┌─────────────────────────────────────────────────────────────┐
...
└────────────────────────┬────────────────────────────────────┘
                         ...
                         v
┌─────────────────────────────────────────────────────────────┐
│  Frontend JS listeners → window.__blackbox_test            │
...
1. 启动 BLACKBOX dev 构建：`pnpm tauri dev`
2. 确认控制台输出：`[BLACKBOX] Test harness registered on /tmp/blackbox-test.sock`
3. 验证连通性：`node scripts/blackbox-cli.mjs ping`（应返回 `{"ok":true,"pong":true}`）
...
```bash
# 检查编辑器
node scripts/blackbox-cli.mjs wait-for --selector '[data-testid=send-button]' --timeout 5000
# → ok：可以 type + send
...
node scripts/blackbox-cli.mjs new-session --cwd ~/Documents
node scripts/blackbox-cli.mjs check-editor   # 确认编辑器可用
node scripts/blackbox-cli.mjs type "hello"
node scripts/blackbox-cli.mjs send

# ✅ 备选：切到已有会话
node scripts/blackbox-cli.mjs switch-session <SESSION_ID>

# ❌ 错误：new-session 无参数后直接 type（欢迎页无编辑器，type 静默失败）
node scripts/blackbox-cli.mjs new-session
node scripts/blackbox-cli.mjs type "hello"  # 无效
```
...
node scripts/blackbox-cli.mjs restart

# 硬重启（杀进程 + 新窗口，仅 Tauri 后端出问题时用）
node scripts/blackbox-cli.mjs relaunch
```

---

## 三、CLI 工具：blackbox-cli.mjs

### 基本用法

```bash
node scripts/blackbox-cli.mjs <command> [args] [--flags]
```
...
# 全局状态快照（最常用的诊断命令）
node scripts/blackbox-cli.mjs status
...
node scripts/blackbox-cli.mjs restart
node scripts/blackbox-cli.mjs restart --timeout 30000
# → {"ok":true,"restarted":true,"mode":"reload","elapsed":10000}

# 硬重启（杀进程 + 重新 spawn，开新窗口，仅 Tauri 后端出问题时使用）
node scripts/blackbox-cli.mjs relaunch
node scripts/blackbox-cli.mjs relaunch --timeout 120000
...
`relaunch` 工作流程：
1. 通过 `lsof -t -U` 查找监听 socket 的进程 PID（尊重 `BLACKBOX_SOCKET` 环境变量）
...
```bash
# 输入文字到 TipTap 编辑器
node scripts/blackbox-cli.mjs type "你好"

# 发送消息（触发 Claude CLI）
node scripts/blackbox-cli.mjs send

# 停止生成
node scripts/blackbox-cli.mjs stop

# 删除当前 session（清理 tab + 终止 CLI 进程，防止 session 累积）
node scripts/blackbox-cli.mjs delete-session
# → {"ok":true,"deleted":true,"session":"desk_..."}

# 读消息（默认最后 10 条）
node scripts/blackbox-cli.mjs get-messages
node scripts/blackbox-cli.mjs get-messages --last 3
node scripts/blackbox-cli.mjs get-messages --all
# → {"ok":true,"messages":[...],"total":16}

# 默认返回摘要（content 截断 150 字，thinking → [thinking]，tool_result → 前 80 字）
# 需要完整内容时加 --full
node scripts/blackbox-cli.mjs get-messages --last 5 --full

# 检查编辑器是否可用（type/send 前必检）
node scripts/blackbox-cli.mjs check-editor
# → {"ok":true,"hasEditor":true,"hasChatPanel":true,"session":"uuid"}

# 检查是否正在生成
node scripts/blackbox-cli.mjs is-streaming
node scripts/blackbox-cli.mjs is-streaming --tab <TAB_ID>
```
...
```bash
node scripts/blackbox-cli.mjs get-active-session
node scripts/blackbox-cli.mjs get-all-sessions
node scripts/blackbox-cli.mjs switch-session <SESSION_ID>  # 完整加载：读 JSONL + 设 working dir + 填充消息
node scripts/blackbox-cli.mjs new-session                  # 无参数：导航到欢迎页（无编辑器）
node scripts/blackbox-cli.mjs new-session --cwd /path/to   # 有 cwd：创建可用新会话（有编辑器）
```
...
```bash
node scripts/blackbox-cli.mjs get-current-model
node scripts/blackbox-cli.mjs get-current-provider
node scripts/blackbox-cli.mjs switch-model claude-sonnet-4-6
node scripts/blackbox-cli.mjs switch-provider <PROVIDER_ID>
node scripts/blackbox-cli.mjs switch-provider null   # 重置为系统默认
```

#### 设置面板

```bash
node scripts/blackbox-cli.mjs open-settings
node scripts/blackbox-cli.mjs close-settings
node scripts/blackbox-cli.mjs switch-settings-tab provider   # general|provider|cli|mcp
```

#### 权限处理

```bash
node scripts/blackbox-cli.mjs allow-permission
node scripts/blackbox-cli.mjs deny-permission
```
...
node scripts/blackbox-cli.mjs get-visible-text
node scripts/blackbox-cli.mjs get-visible-text --selector '[data-testid=chat-messages]'

# 截图（返回文件路径，用 Read 工具查看图片）
node scripts/blackbox-cli.mjs screenshot

# 页面结构/状态/应用信息
node scripts/blackbox-cli.mjs query-page --mode state
node scripts/blackbox-cli.mjs query-page --mode info
node scripts/blackbox-cli.mjs query-page --mode map

# 精简元素查询（仅交互元素 + 不含文本内容，大幅减少输出噪音）
node scripts/blackbox-cli.mjs query-page --mode map --interactive-only
node scripts/blackbox-cli.mjs query-page --mode map --interactive-only --no-content

# 原始 HTML
node scripts/blackbox-cli.mjs get-dom
```
...
```bash
# 等待文本出现
node scripts/blackbox-cli.mjs wait-for --text "收到" --timeout 5000

# 等待元素出现
node scripts/blackbox-cli.mjs wait-for --selector '[data-testid=permission-card]'

# 等待消息生成完成（轮询 status，超时=失败）
node scripts/blackbox-cli.mjs wait-until-done --timeout 60000

# 等待特定 phase（如等模型进入 writing 阶段再中断）
node scripts/blackbox-cli.mjs wait-for-phase writing --timeout 15000
# phases: thinking | writing | tool | awaiting | completed

# 纯延时（不检查状态，用于时序控制）
node scripts/blackbox-cli.mjs delay 3000
```
...
```bash
node scripts/blackbox-cli.mjs exec "document.title"
node scripts/blackbox-cli.mjs exec "JSON.stringify(window.__blackbox_test.status())"
```
...
| 变量 | 默认值 | 说明 |
|------|--------|------|
| `BLACKBOX_SOCKET` | `/tmp/blackbox-test.sock` | 覆盖 socket 路径（restart 命令也遵循） |
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| `--report <path>` | `/tmp/blackbox-test-report-{时间戳}.json` | 报告输出路径 |
...
1. **预检**：启动时 `ping` 检查 BLACKBOX 是否在运行
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `cmd` | string | 是 | blackbox-cli.mjs 命令名 |
| `args` | string[] | 否 | 位置参数 |
...
```json
{
  ...
  "timeout": 90000,
  "setup": [
    {"cmd": "new-session", "flags": {"cwd": "/tmp/blackbox-test"}},
...
| 文件 | 行数 | 说明 |
|------|------|------|
| `scripts/blackbox-cli.mjs` | ~715 | CLI 工具主体，30 个命令（含 restart/relaunch），零外部依赖 |
...
在 `useEffect` 中扩展了 `window.__blackbox_test` 对象（仅 `import.meta.env.DEV` 下生效）：
...
- `pendingPermission` 检查 `window.__blackbox_respond_permission` 是否存在
...
| `src-tauri/src/lib.rs` (`run()` 末尾) | 测试通道注册，在 `/tmp/blackbox-test.sock` 启动 socket server |
...
- **relaunch 进程泄漏**：`relaunch` 通过 `lsof` 找 socket 持有者 PID 来杀进程，但旧的 `target/debug/blackbox` 子进程经常杀不干净。长时间无人值守测试（5 小时）实测累积了 76 个僵尸进程。测试前后手动清理：`pkill -9 -f 'target/debug/blackbox'`。
...
- `window.__blackbox_test` helper 仅在 `import.meta.env.DEV` 下挂载
